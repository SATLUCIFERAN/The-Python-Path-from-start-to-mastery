...
from utils import is_valid_date, format_date, get_month_name
...
class CalendarApp:
    ...
    def on_day_click(self, row, col):
        ...
        if not day_text:
            return
        day = int(day_text)
        date_str = format_date(self.current_year, self.current_month, day)
    ...
    def open_day_dialog(self, date_str):
        appts = fetch_appointments(date_str)
        ...
        def refresh_list():
            ...
            updated = fetch_appointments(date_str)
            for appt in updated:
                appt_id, appt_time, appt_desc = appt
                listbox.insert(tk.END, f"{appt_time}  |  {appt_desc}  (ID:{appt_id})")
        ...
        def on_add():
            ...
            try:
                ...
                messagebox.showerror(
                    "Invalid Time", "Time must be in HH:MM format (24-hour).", parent=dialog
                )
                return
            ...
            add_appointment(date_str, time_input, desc)
            messagebox.showinfo("Saved", "Appointment added.", parent=dialog)
            refresh_list()
            self.populate_calendar(self.current_year, self.current_month)
    ...
    def populate_calendar(self, year, month):
        ...
        days_with_appts = count_appointments_by_day(year, month)
